...
class Transport {
  constructor (opts) {
    ...
    this.observability = opts.observability || new Observability()
    ...
    this[kSniffEnabled] = typeof this.sniffInterval === 'number'
    this[kNextSniff] = this[kSniffEnabled] ? (Date.now() + this.sniffInterval) : 0
  ...
  async request (params, options = {}) {
    ...
    this.observability.emit('serialization', null, result)
    ...
    if (params.body != null) {
      if (shouldSerialize(params.body)) {
        try {
          ...
          this.observability.emit('request', err, result)
          throw err
        }
      }
    ...
    while (meta.attempts <= maxRetries) {
      try {
        ...
        this.observability.emit('request', null, result)
        ...
        //   this.observability.emit('response', null, result)
        //   return result
        // }
        ...
        switch (error.name) {
          ...
          case 'ResponseError':
            if (error.name === 'RequestAbortedError') {
              ...
              this.observability.emit('response', wrappedError, result)
              throw wrappedError
            } else {
              this.observability.emit('response', error, result)
              throw error
            }
          // should retry
          case 'TimeoutError':
  ...
  getConnection (opts) {
    ...
    if (this[kSniffEnabled] && now > this[kNextSniff]) {
      this[kNextSniff] = now + this.sniffInterval
