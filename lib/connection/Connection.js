  ...
  request (params) {
    return new Promise((resolve, reject) => {
      ...
      if (INVALID_PATH_REGEX.test(requestParams.path) === true) {
        reject(new TypeError(`ERR_UNESCAPED_CHARACTERS: ${requestParams.path}`))
      ...
      const onResponse = response => {
        ...
        const onBody = (err, body) => {
          ...
          resolve({ body, statusCode: response.statusCode, headers: response.headers })
        }
      ...
      const onTimeout = () => {
        ...
        request.abort()
        reject(new TimeoutError('Request timed out', params))
      }
